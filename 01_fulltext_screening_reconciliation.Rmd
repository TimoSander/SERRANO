...
reconciledDecisions <- read_csv("data-raw/screeningReconciliation.csv")
```
...
reconciledDecisions <- reconciledDecisions %>%
  ...
  select(Keywords, ScreeningDecisions) %>% # Keywords column contains StudyIdStr
  rename(StudyIdStr = Keywords) # Rename for merging

allScreeningDecisions <- rbind(matchedDecisions, reconciledDecisions) # Merging this way converts TRUE = 1 and FALSE = 0, so data match

# Convert 1 = Included and 0 = Excluded
allScreeningDecisions$ScreeningDecisions <- sub("1", "Included", allScreeningDecisions$ScreeningDecisions)
allScreeningDecisions$ScreeningDecisions <- sub("0", "Excluded", allScreeningDecisions$ScreeningDecisions)
```
